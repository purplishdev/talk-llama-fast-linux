...
static struct ggml_tensor * llm_build_kqv(
    ...
    if (max_alibi_bias > 0.0f) {
		...
		float temp = 1.0; // Example: typical value for softmax temperature
		kq = ggml_soft_max_ext(ctx, kq, kq_mask, nullptr, temp, kq_scale);
...
void llama_backend_init(bool numa) {
	...
	if (numa) {
		ggml_numa_init(GGML_NUMA_STRATEGY_COUNT);
	}
